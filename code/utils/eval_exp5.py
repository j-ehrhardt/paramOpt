...
def retrieve_optimization_results_same_ds(model_type:str, dataset_id:str, dataset_trans:str, testset_selection:str, optimization_setup:str):
    filepath = f'../../exp_hpc/exp3/{dataset_id}/{model_type}/{testset_selection}'

    filename = f'{model_type}_{dataset_id}_{testset_selection}_0_on_{dataset_trans}_{optimization_setup}_is_RMSprop.json'
...
def build_mae_table_per_setup_with_sd(model_type: str, dataset_ids: list[str], testset_selection: str, setups: list[str], dataset_trans: list[str], ddof: int = 1, decimals: int = 1, format_cells: bool = True, verbose: bool = False):
    """
    ...
    for setup in setups:
        ...
        for ds_trans in dataset_trans:
            row = {"setup": setup, "ds_trans": ds_trans}
            ...
            for ds in dataset_ids:
                try:
                    ...
                    if format_cells:
                        ...
                        row[f"{ds}_sd"] = np.nan
                    if verbose:
                        print(f"[missing] model={model_type}, ds={ds}, sel={testset_selection}, setup={setup}, ds_trans={ds_trans} -> {e}")

            rows.append(row)

    df = pd.DataFrame(rows).set_index(["setup", "ds_trans"])
    return df
...
if __name__ == '__main__':
    ...
    #dataset_ids = ['ds4', 'ds6', 'ds8']
    dataset_ids = ['ds1', 'ds3', 'ds5'] #, 'ds4', 'ds6', 'ds8']
    dataset_trans = dataset_ids


    setups = ['[True, False, False]', '[False, True, False]', '[False, True, True]', '[True, True, True]']


    df1 = build_mae_table_per_setup_with_sd(model_type='res', dataset_ids=dataset_ids, testset_selection='end', setups=setups, dataset_trans=dataset_trans)
